...
from data_loader import NERDataset, PretrainDataset
...
class FGM(object):
    ...
    def __init__(self, model, emb_name, epsilon=1.0e-3):
        ...
        for name, param in self.model.named_parameters():
            if param.requires_grad and self.emb_name in name:
                ...
                if norm != 0 and not torch.isnan(norm):
                    ...
                    logging.info("--------Attack the {}--------".format(name))
...
def test():
    ...
    if config.model_dir is not None:
        model = BertNER.from_pretrained(config.test_model)
        model.to(config.device)
        logging.info("--------Load model from {}--------".format(config.test_model))
    else:
        logging.info("--------No model to test !--------")
        return
    predict(test_loader, model, mode='test')
    '''
...
def pretrain():
    ...
    # 导入预训练文本数据
    data = np.load(config.pretrain_dir, allow_pickle=True)
    words = data['words']
    # build dataset
    train_dataset = PretrainDataset(words, config)
    logging.info("--------Dataset Build!--------")
    # get dataset size
