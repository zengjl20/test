...
def predict(dev_loader, model, mode='dev'):
    # set model to evaluation mode
    model.eval()
    if mode == 'test':
        tokenizer = BertTokenizer.from_pretrained(config.bert_model, do_lower_case=True, skip_special_tokens=True)
    ...
    with torch.no_grad():
        for idx, batch_samples in enumerate(dev_loader):
            ...
            pred_tags.extend([[id2label.get(idx) for idx in indices] for indices in batch_output])
            true_tags.extend([[id2label.get(idx) for idx in indices if idx > -1] for indices in batch_tags])
            with open('./submit4.csv', 'r+', encoding='utf-8') as f:
                row = csv.writer(f)
                row.writerow(['id','tag'])
                i = 0
                for id, tag in enumerate(pred_tags):
                    for t in tag:
                        row.writerow([i, t])
                        #print("{},{}".format(i, t))
                        i += 1
                    if true_tags[id][-1] == 'O':
                        row.writerow(['',''])
                if i % 1000 == 0:
                    logging.info("--------have finished {} notes!--------".format(i))
            # (batch_size, max_len - padding_label_len)
            #true_tags.extend([[id2label.get(idx) for idx in indices if idx > -1] for indices in batch_tags])
